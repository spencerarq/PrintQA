# alembic/env.py
import os
from dotenv import load_dotenv
from sqlalchemy import engine_from_config, pool
from alembic import context
from logging.config import fileConfig

# Carrega variáveis de ambiente do .env para Alembic
load_dotenv()

config = context.config # type: ignore
if config.config_file_name is not None:
    fileConfig(config.config_file_name)

from printqa.database import Base 
from printqa import models
target_metadata = Base.metadata

def get_database_url():
    """
    Constrói a URL do banco de dados a partir das variáveis de ambiente
    """
    db_user = os.getenv("DB_USER", "root")
    db_password = os.getenv("DB_PASSWORD")
    db_host = os.getenv("DB_HOST", "db")
    db_port = os.getenv("DB_PORT", "3306")
    db_name = os.getenv("DB_NAME", "printqa_db")
    
    if not db_password:
        raise ValueError("DB_PASSWORD não está definida nas variáveis de ambiente")
    
    return f"mysql+mysqlconnector://{db_user}:{db_password}@{db_host}:{db_port}/{db_name}"

def run_migrations_offline() -> None:
    """Run migrations in 'offline' mode.

    Este modo configura o contexto apenas com uma URL
    e não um Engine, embora um Engine também seja aceitável
    aqui. Ao pular a criação do Engine,
    não precisa que uma DBAPI esteja disponível.

    Chamadas para context.execute() aqui emitem a string fornecida para
    a saída do script.
    """
    url = get_database_url()
    context.configure(
        url=url,
        target_metadata=target_metadata,
        literal_binds=True,
        dialect_opts={"paramstyle": "named"},
    )
    
    with context.begin_transaction():
        context.run_migrations()

def run_migrations_online() -> None:
    """Run migrations in 'online' mode.

    Neste cenário, precisa criar um Engine
    e associar uma conexão com o contexto.
    """
    database_url = get_database_url()
    config.set_main_option("sqlalchemy.url", database_url) # type: ignore
    
    connectable = engine_from_config(
        config.get_section(config.config_ini_section),
        prefix="sqlalchemy.",
        poolclass=pool.NullPool,
    )

    with connectable.connect() as connection: # type: ignore
        context.configure(
            connection=connection, 
            target_metadata=target_metadata
        )

        with context.begin_transaction():
            context.run_migrations()

if context.is_offline_mode():
    run_migrations_offline()
else:
    run_migrations_online()